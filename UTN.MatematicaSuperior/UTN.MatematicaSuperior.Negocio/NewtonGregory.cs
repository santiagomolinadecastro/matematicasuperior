...
namespace UTN.MatematicaSuperior.Negocio
{
    public static class NewtonGregory
    {
        public static string Interpolar(List<double> xList, List<double> yList)
        {
            ...
            double[] coeficientes = ObtenerCoeficientesProgresivo(y, n);

            //Creo un polinomio con el término independiente.
            Polynomial polinomio = new Polynomial(new List<double> { coeficientes[0] });
            ...
            for (int i = 1; i < n; i++)
            {
                ...
                Polynomial polinomioTemp = new Polynomial(new List<double> { -coeficientes[1], 1 });

                //Multiplico los demás (x-a), en caso de haber, al polinomio temporal.
                for (int j = 1; j < i; j++)
                {
                    polinomioTemp *= new Polynomial(new List<double> { -coeficientes[j + 1], 1 });
                }

                //Le sumo al polinomio la multiplicación entre el término del árbol y el polinomio obtenido.
                polinomio += new Polynomial(new List<double> { x[i] }) * polinomioTemp;
            }
